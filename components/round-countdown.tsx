...
export function RoundCountdown({
  ...
  if (!now)
    return (
      <div
        className="countdown-skeleton"
        aria-label="Loading voting countdown"
      />
    );
  ...
  if (!open) return <div className="round-closed">VOTING CLOSED</div>;
  return (
    <div className="countdown-wrap">
      ...
      <div className="week-progress">
        ...
        <span>
          Day {Math.min(7, Math.floor((now - start) / 86400000) + 1)} of 7
        </span>
      </div>
    </div>
  );
